  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Rate Your Session">
      <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium text-zinc-700 mb-3 text-center">Rating</label>
              ...
              >
                <Star
                  className={cn(
                    ...
                    (hoveredStar >= rating || selectedStars >= rating) ? 'fill-yellow-400 text-yellow-400' : 'text-zinc-300'
                  )}
                />
              </button>
            ))}
          </div>
          {errors.stars && <p className="mt-2 text-sm text-red-600 text-center">{errors.stars.message}</p>}
        </div>

        <Textarea label="Feedback (Optional)" rows={4} placeholder="Share your experience..." error={errors.feedback?.message} {...register('feedback')} />

        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-blue-800">
            <span className="font-medium">{recipientName}</span> will earn <span className="font-bold">{selectedStars * 10} points</span> for this session
          </p>
        </div>
